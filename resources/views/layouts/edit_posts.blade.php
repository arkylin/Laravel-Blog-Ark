...
<script>
    ...
    function GetFanyi (query) {
        var appid = '<?php echo env('FANYI_APPID') ?>';
        var key = '<?php echo env('FANYI_KEY') ?>';
        ...
        $.ajax({
            ...
            success: function (data) {
                var data_p = data['trans_result'][0]['dst'];
                $("#post_slug").val(data_p.toLowerCase().replace(/[^0-9&A-z&-]/ig, '-').replace(new RegExp('\-+',"gm"),'-').replace(/(.*)\-/, "$1"));
            } 
        });
        // return final_d;
    }
